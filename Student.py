...
class Student:
    ...
    def rate_lecturer(self, lecturer, course_name, grade):    # оценка студентами лекторов
        if isinstance(lecturer, Lecturer) and course_name in self.courses_in_progress \
                and course_name in lecturer.courses_attached and 1 <= grade <= 10:    # grade передается при обращении к методу
...
class Reviewer(Mentor):		#Создаем дочерний класс Reviewer от родительского Mentor
    ...
    def rate_hw(self, student, course, grade):    #если студент и курс есть в списке курсов
...
cool_reviewer.rate_hw(bad_student, 'Java', 10)
cool_reviewer.rate_hw(bad_student, 'Java', 10)
cool_reviewer.rate_hw(bad_student, 'Java', 10)
...
bad_student.rate_lecturer(def_lecturer, "Python", 8)
bad_student.rate_lecturer(def_lecturer, "Python", 9)
...
print(best_student)     
print()
print (f"Средняя оценка за домашнее задание по всем студентам : {avg_grade_all_students(students_list, 'Python')}")   
print (f"Средняя оценка за лексции всех лекторов : {avg_grade_all_lecturers(lecturers_list, 'Python')}")        